...
//void
//InputsConfigure (InputNameType InName, uint8_t InDebounceDown, uint8_t InDebounceUp, GPIO_TypeDef* GPIO, uint16_t GPIO_Pin);
...
InputsPeriodic (void)
{
	...
	for (i = 0; i < kInputCount; i++)
	{
		...
		if ((InputConf[i].InState == GPIO_PIN_RESET))// && (InputConf[i].InStateOld == GPIO_PIN_SET))
		{
...
//			else
		...
		if ((InputConf[i].InState == GPIO_PIN_SET))// && (InputConf[i].InStateOld == GPIO_PIN_RESET))
		{
